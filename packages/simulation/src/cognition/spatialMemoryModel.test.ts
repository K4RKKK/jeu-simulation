...
describe('rememberSpatial', () => {
  it('ajoute un nouveau souvenir avec la confiance/précision fraîches', () => {
    ...
    rememberSpatial(memory, { kind: 'water', x: 10, z: 20, source: 'selfExperience' }, 100, config);
  ...
  it('rafraîchit un souvenir revu au même endroit plutôt que de le dupliquer', () => {
    ...
    rememberSpatial(
      memory,
      { kind: 'water', x: 10.1, z: 20.1, source: 'selfExperience' },
      500,
      config,
    );
  ...
  it('identifie deux souvenirs distincts par worldRef, même à la même position', () => {
    ...
    rememberSpatial(
      memory,
      { kind: 'resource', x: 5, z: 5, worldRef: worldRefA, source: 'selfExperience' },
      1,
      config,
    );
    rememberSpatial(
      memory,
      { kind: 'resource', x: 5, z: 5, worldRef: worldRefB, source: 'selfExperience' },
      1,
      config,
    );
  ...
  it('évince le souvenir le moins confiant quand la capacité est dépassée', () => {
    ...
    for (let i = 0; i < config.maxSpatialEntries; i++) {
      rememberSpatial(
        memory,
        { kind: 'place', x: i * 100, z: 0, source: 'selfExperience' },
        1,
        config,
      );
    }
    ...
    rememberSpatial(memory, { kind: 'place', x: 999, z: 0, source: 'selfExperience' }, 1, config);
  ...
  it('alloue des MemoryId séquentiels via le compteur du composant', () => {
    const memory = createEmptyCognitiveMemory();
    rememberSpatial(memory, { kind: 'water', x: 0, z: 0, source: 'selfExperience' }, 1, config);
    rememberSpatial(memory, { kind: 'water', x: 1000, z: 0, source: 'selfExperience' }, 1, config);
...
describe('decaySpatialMemory', () => {
  it("ne fait rien tant qu'aucun temps ne s'est écoulé", () => {
    const memory = createEmptyCognitiveMemory();
    rememberSpatial(memory, { kind: 'water', x: 0, z: 0, source: 'selfExperience' }, 100, config);
  ...
  it('divise la confiance par deux après exactement une demi-vie', () => {
    const memory = createEmptyCognitiveMemory();
    rememberSpatial(memory, { kind: 'water', x: 0, z: 0, source: 'selfExperience' }, 0, config);
  ...
  it('donne le même résultat qu’on décroisse en une fois ou en plusieurs passes intermédiaires', () => {
    ...
    rememberSpatial(a, { kind: 'water', x: 0, z: 0, source: 'selfExperience' }, 0, config);
    decaySpatialMemory(a, 20000, 0.05, config);

    const b = createEmptyCognitiveMemory();
    rememberSpatial(b, { kind: 'water', x: 0, z: 0, source: 'selfExperience' }, 0, config);
